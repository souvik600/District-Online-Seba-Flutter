...
class AdminParkResortScreen extends StatefulWidget {
  @override
  _AdminParkResortScreenState createState() => _AdminParkResortScreenState();
}

class _AdminParkResortScreenState extends State<AdminParkResortScreen> {
  ...
  // Add hospital
  Future<void> _addParkResort() async {
    final data = {
    ...
    final docRef = await _firestore.collection('ParkResort').add(data);
  ...
  // Update hospital
  Future<void> _updateParkResort(String id) async {
    final data = {
      'name': _nameController.text.trim(),
      'location': _locationController.text.trim(),
      'contact': _contactController.text.trim(),
    ...
    await _firestore.collection('ParkResort').doc(id).update(data);
  ...
  Future<String> _uploadImage(String id) async {
    final ref = _storage.ref().child('ParkResort_images/$id.jpg');
  ...
  Future<void> _deleteParkResort(String id) async {
    await _firestore.collection('ParkResort').doc(id).delete();
  }

  // Show form for adding or editing hospital
  void _showParkResortForm({String? id, Map<String, dynamic>? data}) {
          ...
          child: Container(
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  Text(
                    data == null ? 'Add Park & Resort' : 'Edit Park & Resort',
                  ...
                  TextField(
                      ...
                      decoration: AppInputDecoration('Park & Resort  Name')),
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      onPressed: () {
                        if (data == null) {
                          _addParkResort(); // Add hospital if data is null
                        } else {
                          _updateParkResort(
              ...
              Expanded(
                child: StreamBuilder(
                  ...
                  builder: (context, snapshot) {
                    ...
                    final ParkResorts = snapshot.data!.docs;
                    return ListView.builder(
                      itemCount: ParkResorts.length,
                      itemBuilder: (context, index) {
                        final data = ParkResorts[index].data();
                        return Padding(
                          padding: const EdgeInsets.all(8.0),
                          child: Stack(
                            children: [
...
class ParkResortDetailsScreen extends StatelessWidget {
  final Map<String, dynamic> data;

  ParkResortDetailsScreen({required this.data});
  ...
  // Hospital Name and Location
  Widget _buildParkResortNameAndLocation() {
    return Card(
      elevation: 12,
