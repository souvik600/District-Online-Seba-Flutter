...
class AdminPrimarySchoolScreen extends StatefulWidget {
  @override
  _AdminPrimarySchoolScreenState createState() =>
      _AdminPrimarySchoolScreenState();
}

class _AdminPrimarySchoolScreenState extends State<AdminPrimarySchoolScreen> {
  ...
  Future<void> _addSchool() async {
    ...
    final docRef = await _firestore.collection('primarySchool').add(data);
  ...
  Future<void> _updateSchool(String id) async {
    ...
    await _firestore.collection('primarySchool').doc(id).update(data);

    if (_selectedImage != null) {
      final imageUrl = await _uploadImage(id);
      await _firestore
          .collection('primarySchool')
          .doc(id)
  ...
  Future<String> _uploadImage(String id) async {
    final ref = _storage.ref().child('primary_school_images/$id.jpg');
  ...
  Future<void> _deleteSchool(String id) async {
    await _firestore.collection('primarySchool').doc(id).delete();
  }
                                  ...
                                  child: Container(
                                    ...
                                    decoration: BoxDecoration(
                                      image: const DecorationImage(
                                        image:
                                        AssetImage('assets/icons/primary-school.png'),
                                        fit: BoxFit.contain,
                                      ),
                                      borderRadius: BorderRadius.circular(8.0),
                                    ),
