...
import 'package:district_online_service/Screens/AdminPanel/AdminHomeScreen/AdminCategoryPage/AdminCommonServiceCategory/AdminRestarurantScreen/restarurent_details_screen.dart';
...
class _AdminRestaurantScreenState extends State<AdminRestaurantScreen> {
  ...
  Future<void> _addRestaurant() async {
    ...
    final docRef = await _firestore.collection('restaurant').add(data);
  ...
  Future<void> _updateRestaurant(String id) async {
    final data = {
      'name': _nameController.text.trim(),
      'location': _locationController.text.trim(),
      'contact': _contactController.text.trim(),
    ...
    await _firestore.collection('restaurant').doc(id).update(data);

    if (_selectedImage != null) {
      final imageUrl = await _uploadImage(id);
      await _firestore.collection('restaurant').doc(id).update({'image': imageUrl});
    }
  ...
  Future<String> _uploadImage(String id) async {
    final ref = _storage.ref().child('restaurant_images/$id.jpg');
  ...
  Future<void> _deleteRestaurant(String id) async {
    await _firestore.collection('restaurant').doc(id).delete();
  }

  void _showRestaurantForm({String? id, Map<String, dynamic>? data}) {
    if (data == null) {
          ...
          child: Container(
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  Text(
                    data == null ? 'Add Restaurant' : 'Edit Restaurant',
                  ...
                  TextField(
                    ...
                    decoration: AppInputDecoration('Restaurant Name'),
                  ),
                  const SizedBox(height: 8.0),
                  TextField(
                    controller: _locationController,
                    decoration: AppInputDecoration('Restaurant Location'),
                  ),
                  const SizedBox(height: 8.0),
                  TextField(
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      ...
                      child: Text(
                        data == null ? 'Add Restaurant' : 'Update Restaurant',
  ...
  void _openDetailsScreen(Map<String, dynamic> data) {
    Navigator.of(context)
        .push(MaterialPageRoute(builder: (_) => RestaurantDetailsScreen(data: data)));
  }
  @override
                  ...
                  builder: (context, snapshot) {
                    ...
                    return ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        final data = restaurants[index].data();
                        return Padding(
                          padding: const EdgeInsets.all(8.0),
                          child: Stack(
                            children: [
                                  ...
                                  child: Column(
                                    children: [
                                      ...
                                      Row(
                                        children: [
                                          TextButton(
                                            onPressed: () => _showRestaurantForm(id: restaurants[index].id, data: data),
                                            child: const Text('Edit'),
                                          ),
                                          TextButton(
                                            onPressed: () => _deleteRestaurant(restaurants[index].id),
